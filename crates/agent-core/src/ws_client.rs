...
use common::messages::{
    ConfigPush, ConfigReload, FetchLogs, LockNow, MSG_CONFIG_PUSH, MSG_CONFIG_RELOAD,
    MSG_FETCH_LOGS, MSG_LOCK_NOW, MSG_NOTIFY_USER, MSG_PAIRING_ACCEPTED, MSG_REMAINING_UPDATE,
    MSG_UNPAIR, MSG_UPDATE_AGENT, NotifyUser, PairingAccepted, RemainingUpdate, ServerMessage,
    Unpair, UpdateAgent,
};
...
pub async fn send<T: serde::Serialize>(
    ...
    tx.send(msg)
        .await
        .context("WebSocket send channel closed")?;
    Ok(())
}
